...
function ticket(doc) {
	var createdBy = doc.createdBy ? doc.createdBy : Meteor.userId;
	var ticket = {
		title: doc.title,
		description: doc.description,
		createdDate: doc.createdDate || moment().format('M/D/YYYY'),
		createdBy: createdBy,
		categoryIds: doc.categoryIds || [], 
		assignedToIds: doc.assignedToIds || [],
		dueDate: doc.dueDate || 'No due date set',
...
function notification(doc) {
	var duration = doc.duration || 1000;
	var notification = {
		message: doc.message,
		className: doc.className,
		save: function() {
			var id = Collections.Notifications.insert(this);
			setTimeout(function(){
				Collections.Notifications.remove({ _id: id });
			}, duration);
			console.log("saving notification", Collections.Notifications.findOne({ _id: id }));
		}
	}
	return notification;
}